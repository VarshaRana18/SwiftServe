...
export default function GlobalHeader({ setActiveRestaurantId }) {
    const { setIsCartOpen, setActiveCartView, globalCarts, setIsOrdersOpen, setIsSettingsOpen, userProfile } = useCart();
    ...
    const searchResults = searchQuery.trim() === "" ? [] : restaurants.reduce((acc, rest) => {
        ...
        const matchesNameOrTag = rest.name.toLowerCase().includes(query) || 
                                 rest.tags.some(tag => tag.toLowerCase().includes(query));
        
        // 2. Check deep-level matches (Menu Items)
        const restMenu = mockMenus[rest.id] || [];
        const matchingItems = restMenu.filter(item => 
        ...
        if (matchesNameOrTag || matchingItems.length > 0) {
            acc.push({
                ...
                matchReason: !matchesNameOrTag && matchingItems.length > 0 
                    ? `Matches: ${matchingItems[0].name}` 
                    : null
            });
        }
        return acc;
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between gap-4">
                    ...
                    <div className="flex-1 max-w-xl relative">
                        <div className="relative">
                            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onFocus={() => setIsSearchFocused(true)} onBlur={() => setTimeout(() => setIsSearchFocused(false), 200)} onKeyDown={handleKeyDown} placeholder="Search for restaurants, cuisines, or dishes..." className="w-full bg-slate-100 text-slate-800 rounded-xl pl-12 pr-4 py-3 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:bg-white transition-all border border-transparent focus:border-orange-500 shadow-inner" />
                                ...
                                <div className="p-2">
                                    ...
                                    {searchResults.length > 0 ? (
                                        searchResults.map((result, index) => (
                                            <button key={result.id} onClick={() => { setActiveRestaurantId(result.id); setSearchQuery(""); }} className={`w-full text-left px-3 py-3 rounded-lg flex items-center justify-between transition-colors ${focusedIndex === index ? 'bg-slate-100' : 'hover:bg-slate-50'}`}>
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="flex flex-col">
                                                        ...
                                                        {/* THE UI HINT: Conditionally render the tag or the deep match reason */}
                    ...
                    <div className="flex items-center gap-4 relative">
                        ...
                        <div onClick={() => setIsProfileOpen(!isProfileOpen)} className="w-10 h-10 bg-slate-200 rounded-full border-2 border-slate-200 overflow-hidden cursor-pointer hover:border-orange-500 transition-colors">
                            <img src={`https://ui-avatars.com/api/?name=${encodeURIComponent(userProfile.name)}&background=f97316&color=fff&bold=true`} alt="Profile" />
                        </div>
                                ...
                                <div className="absolute top-full right-0 mt-4 w-64 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden z-50 animate-slide-up">
                                    <div className="p-4 bg-slate-50 border-b border-slate-100 flex items-center gap-3">
                                        ...
                                        <div className="flex flex-col overflow-hidden">
                                            <span className="font-bold text-slate-800 truncate">{userProfile.name}</span>
