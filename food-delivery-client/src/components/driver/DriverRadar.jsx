...
export default function DriverRadar({ isOnline, onAcceptOrder }) {
    const [incomingOrder, setIncomingOrder] = useState(false);
    const [timer, setTimer] = useState(100);

    // Engine 1: Simulate receiving a ping 3 seconds after going online
    useEffect(() => {
        let pingTimeout;
        if (isOnline && !incomingOrder) {
            pingTimeout = setTimeout(() => {
                setIncomingOrder(true);
                setTimer(100); // Reset timer to 100%
            }, 3000);
        } else if (!isOnline) {
    ...
    // Engine 2: The 10-second countdown bar
    useEffect(() => {
        let countdown;
        if (incomingOrder) {
            countdown = setInterval(() => {
                setTimer(prev => {
                    if (prev <= 1) {
                        ...
                        setIncomingOrder(false); // Auto-decline if missed
                        return 0;
                    }
                    return prev - 1;
                });
            }, 100); // Ticks down 1% every 100ms (10 seconds total)
        }
    ...
    const handleAccept = () => {
        setIncomingOrder(false);
        onAcceptOrder(); // Tell the parent layout to switch tabs
    };
    ...
    return (
        <div className="h-full flex flex-col relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col h-full justify-between p-6">
                ...
                {/* Radar Animation (Only when online) */}
                ...
                <div className="bg-white p-5 rounded-2xl shadow-lg shadow-slate-200/50 border border-slate-100 flex justify-between items-center mb-6 opacity-90">
                    <div className="flex flex-col">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Earnings</span>
                        <span className="font-extrabold text-slate-800 text-xl">₹850</span>
                    </div>
                    <div className="w-px h-10 bg-slate-100"></div>
                    <div className="flex flex-col">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Deliveries</span>
                        <span className="font-extrabold text-slate-800 text-xl text-center">12</span>
                    </div>
                    <div className="w-px h-10 bg-slate-100"></div>
                    <div className="flex flex-col items-end">
                        ...
                        <span className="font-extrabold text-blue-600 text-xl flex items-center gap-1">
                            4.9 <svg className="w-4 h-4 fill-current" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                        </span>
                    </div>
                </div>
                ...
                <div className="absolute inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex flex-col justify-end pb-6 px-4 animate-fade-in">
                    <div className="bg-white rounded-3xl shadow-2xl p-6 border-b-4 border-b-blue-600 transform translate-y-0 animate-[slideUp_0.3s_ease-out]">
                        ...
                        <div className="flex justify-between items-start mb-4">
                            <span className="bg-blue-100 text-blue-700 text-xs font-bold uppercase tracking-wider px-3 py-1.5 rounded-lg animate-pulse">New Request</span>
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-4">
                            <button onClick={handleDecline} className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-4 rounded-2xl transition-colors">
                                Decline
                            </button>
                            <button onClick={handleAccept} className="bg-blue-600 hover:bg-blue-700 text-white font-extrabold py-4 rounded-2xl shadow-lg shadow-blue-600/30 transition-transform active:scale-95">
                                Accept
                            </button>
                        </div>
                        ...
                        <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
                            <div 
                                className={`h-1.5 rounded-full transition-all duration-100 ease-linear ${timer > 50 ? 'bg-blue-500' : timer > 25 ? 'bg-orange-500' : 'bg-red-500'}`} 
                                style={{ width: `${timer}%` }}
                            ></div>
                        </div>
